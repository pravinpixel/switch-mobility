                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    ...
                                    @if (auth()->user()->is_super_admin == 1 || auth()->user()->can('department-create'))
                                        ...
                                        <a href="{{ url('department/create') }}">
                                            <button type="button" class="btn switchPrimaryBtn">+ Add</button>
                                        </a>
                                    @endif
                                    <!--end::Add user-->
                                </div>
                            ...
                            <table class="table" id="service_table">
                                ...
                                <tbody class="text-gray-600 fw-semibold" id="tableContent">
                                    ...
                                    @foreach ($departments as $key => $d)
                                        <tr height="100px">
                                            <td style="text-align: left!important">{{ htmlspecialchars($d['name']) }}</td>
                                            <td class="text-left" style="max-width:300px !important;">
                                                {{ htmlspecialchars($d['description']) }}</td>
                                                ...
                                                <div class="">
                                                    ...
                                                    @if (auth()->user()->is_super_admin == 1 || auth()->user()->can('department-edit'))
                                                        <div style="display:inline;cursor: pointer;"
                                                            id="{{ $d['id'] }}" class="editDept"
                                                            title="Edit Department"><i class="fa-solid fa-pen"
                                                                style="color:orange"></i></div>
                                                    @endif
                                                    @if (auth()->user()->is_super_admin == 1 || auth()->user()->can('department-delete'))
                                                        <div onclick="delete_item({{ $d['id'] }});"
                                                            style="display:inline;cursor: pointer; margin-left: 10px;"
                                                            id="{{ $d['id'] }}" class=""
                                                            title="Delete Department"><i class="fa-solid fa-trash"
...
<script>
    ...
    $(document).on('change', '.status', function() {
        ...
        var activeStatus = status ? "Active" : "InActive";
    ...
    function getListData() {
        ...
        $.ajax({
            ...
            success: function(result) {
                table.clear(); // Clear all rows without redrawing

                $.each(result.data, function(key, val) {
                    var name = escapeHTML(val.name);
                    var description = escapeHTML(val.description);
                    var id = val.id;
                    var statusRes = (val.is_active == 1) ? "checked" : "";
                    var statusBtn = '<label class="switch">';
                    ...
                    if (isSuperAdmin || isAuthorityEdit) {
                        EditBtn = '<div style="display:inline;cursor: pointer;" id="' + id +
                            '" class="editDept" title="Edit Department"><i class="fa-solid fa-pen" style="color:orange"></i></div>';
                    }

                    if (isSuperAdmin || isAuthorityDelete) {
                        deleteBtn = '<div onclick="delete_item(' + id +
                            ');" style="display:inline;cursor: pointer; margin-left: 10px;" id="' +
                            id +
                            '" class="" title="Delete Department"><i class="fa-solid fa-trash" style="color:red"></i></div>';
                    }

                    var actionBtn = EditBtn + deleteBtn;
                    table.row.add([name, description, statusBtn, actionBtn]);
                });
    ...
    // Example function to escape HTML characters
