                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    ...
                                    @if (auth()->user()->is_super_admin == 1 || auth()->user()->can('designation-create'))
                                        ...
                                        <a href="{{ url('designation/create') }}">
                                            <button type="button" class="btn switchPrimaryBtn" data-bs-toggle="modal"
                        ...
                        <div class="card-body p-3">
                            <div class="card-title" style="display:none">
                                ...
                                <div class="d-flex align-items-center position-relative my-1">
                                    ...
                                    <!--end::Svg Icon-->
                                    <input type="text" class="form-control form-control-solid w-250px ps-14 search"
                                        placeholder="Search" />
                                </div>
                                <!--end::Search-->
                            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex my-3 ms-9">
                                                    <!--begin::Edit-->
                                                    @if (auth()->user()->is_super_admin == 1 || auth()->user()->can('designation-edit'))
                                                        <a class="editPage" style="display:inline;cursor: pointer;"
                                                            id="{{ $d['id'] }}" title="Edit Designation"><i
                                                                class="fa-solid fa-pen" style="color:orange"></i></a>
                                                    @endif
                                                    @if (auth()->user()->is_super_admin == 1 || auth()->user()->can('designation-delete'))
                                                        <div onclick="delete_item(<?php echo $d['id']; ?>);"
                                                            style="display:inline;cursor: pointer; margin-left: 10px;"
                                                            id="{{ $d['id'] }}" class=""
                                                            title="Delete Designation"><i class="fa-solid fa-trash"
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                $.each(result.data, function(key, val) {
                    ...
                    // var id = val.id;
                    var name = DOMPurify.sanitize(val.name);
                    var description = DOMPurify.sanitize(val.description);
                    var id = DOMPurify.sanitize(val.id);
                    ...
                    if (isSuperAdmin || isAuthorityEdit) {
                        // var editBtn = (
                        //     '<a href="' + editurl +
                        //     '" class="btn btn-icon btn-active-light-primary w-30px h-30px me-3"><i class="fa-solid fa-pen" style="color:orange"></i></a>'
                        // );
                        var editBtn = $('<a>', {
                            'href': editurl,
                            'class': 'btn btn-icon btn-active-light-primary w-30px h-30px me-3',
                            'html': '<i class="fa-solid fa-pen" style="color:orange"></i>'
                        });
                    }

                    if (isSuperAdmin || isAuthorityDelete) {
                        // var deleteBtn = '<div onclick="delete_item(' + id +
                        ...
                        var deleteBtn = $('<div>', {
                            ...
                            'html': '<i class="fa-solid fa-trash" style="color:red"></i>'
                        });
                    }
                    ...
                    table.row.add([
                        ...
                        statusBtn,
                        $('<div>').append(editBtn, deleteBtn).html()
                    ]).draw();
                });
            }
        });
